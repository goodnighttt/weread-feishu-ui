...
function buildDirectChapterUrl(chapterUid: string): string {
...
export async function navigateToReaderTocItem(index: number, title: string): Promise<boolean> {
  ...
  // Prefer WeRead's own catalog interaction when its live DOM is available.
  ...
  // Official catalog gives us a stable chapterUid even when the native panel is unavailable.
  ...
  // Current WeRead reader accepts the raw chapterUid as progressChapterUid.
  if (targetUid) {
    const url = buildDirectChapterUrl(targetUid);
    if (url) {
      location.assign(url);
      return true;
    }
  ...
  // Only trust real reader links; ignore # / javascript: pseudo links from native controls.
